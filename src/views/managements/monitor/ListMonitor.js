...
import { GetListMonitor, DeleteMonitor, GetListMonitorByOrganizationId } from '../../../api/monitorAPI'
...
const ListMonitor = () => {
    ...
    const getData = () => {
        GetListMonitor(dataPage.pageSize, dataPage.pageNumber, dataPage.querySearch).then(result => {
            setListMonitor(result)
        }).catch(error => {
            console.log(error)
        })
    }
    ...
    const handleDeleteModal = (id) => {
        return Swal.fire({
            title: '',
            text: 'Bạn có muốn xóa cán bộ này không?',
            icon: 'warning',
    ...
    const handleChangeOrganization = (value) => {
        if (value.value !== 0) {
            GetListMonitorByOrganizationId(value.value, dataPage.pageSize, dataPage.pageNumber).then(result => {
                setListMonitor(result)
            }).catch(error => {
                console.log(error)
            })
    ...
    const handleChangeSearch = (e) => {
        ...
        // GetListMonitor(dataPage.pageSize, dataPage.pageNumber, querySearch).then(result => {
        //     setListMoniGetListMonitor(result)
    ...
    const CustomPagination = () => (
        <ReactPaginate
            ...
            // pageCount={dataPage.querySearch.length ? Math.ceil(filteredData.count / 10) : Math.ceil(listMonitor.count / 10) || 1}
            pageCount={Math.ceil(listMonitor.count / 10) || 1}
            breakLabel={'...'}
    ...
    return (
        <>
            <Breadcrumb className='breadcrumb__page'>
                <BreadcrumbItem>
                    <Link to='/managements/home/home_dashboard'> Trang chủ </Link>
                </BreadcrumbItem>
                <BreadcrumbItem>
            ...
            <Card className='card__monitor'>
                <CardHeader className='border-bottom'>
                    <CardTitle tag='h4'>Danh sách cán bộ</CardTitle>
                    <div className='d-flex mt-md-0 mt-1'>
                        <Button onClick={handleAddModal} className='ms-2' color='primary'>
                            <Plus size={15} />
                            <span className='align-middle ms-50'>Thêm cán bộ</span>
                        </Button>
                    </div>
                </CardHeader>
                ...
                <div className='react-dataTable whole-table__monitor'>
                    <DataTable
                        noHeader
                        pagination
                        selectableRowsNoSelectAll
                        columns={ListMonitorColumns}
                        className='react-dataTable table__monitor'
                        paginationPerPage={7}
                        sortIcon={<ChevronDown size={10} />}
                        paginationDefaultPage={currentPage + 1}
                        paginationComponent={CustomPagination}
                        // data={dataPage.querySearch.length ? filteredData.list : listMonitor.list}
                        data={listMonitor.list}
                    />
                </div>
            </Card>
            <AddMonitor open={addModal} handleAddModal={handleAddModal} getData={getData} />
            {editMonitor && <EditMonitor open={editModal} handleCloseModal={handleCloseModal} handleEditModal={handleEditModal} editMonitor={editMonitor} getData={getData} />}
            {/* <DeleteMonitor open={deleteModal} handleDeleteModal={handleDeleteModal} /> */}
        </>
    )
}
