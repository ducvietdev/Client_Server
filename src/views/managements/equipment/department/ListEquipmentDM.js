...
import AddEquipmentDM from './AddEquipmentDM'
import DeleteEquipmentDM from './DeleteEquipmentDM'
import EditEquipmentDM from './EditEquipmentDM'
import { useNavigation } from '../../../../utility/hooks/useNavigation'
import Swal from 'sweetalert2'
import { DeleteEquipmentType, GetListEquipmentType } from '../../../../api/equipmentAPI'
...
const ListEquipmentDM = () => {
    // ** Data
    const ListEquipmentDMColumns = [
        {
            name: 'ID',
            sortable: false,
            maxWidth: '5%',
    ...
    const getData = () => {
        GetListEquipmentType(dataPage.pageSize, dataPage.pageNumber, dataPage.querySearch).then(result => {
            setListEquipmentType(result)
        }).catch(error => {
            console.log(error)
        })
    }
    ...
    const handleDeleteModal = (id) => {
        return Swal.fire({
            ...
            if (result.value) {
                DeleteEquipmentType(id).then(result => {
                    if (result) {
                        Swal.fire({
                            icon: 'success',
                            title: 'Đã xóa!',
    ...
    const handleChangeSearch = (e) => {
        ...
        // GetListEquipmentType(dataPage.pageSize, dataPage.pageNumber, querySearch).then(result => {
    ...
    const CustomPagination = () => (
        <ReactPaginate
            ...
            // pageCount={dataPage.querySearch.length ? Math.ceil(filteredData.count / 10) : Math.ceil(listEquipmentType.count / 10) || 1}
            pageCount={Math.ceil(listEquipmentType.count / 10)}
            breakLabel={'...'}
    ...
    return (
        <>
            ...
            <Card className='card__equipmentDM'>
                <CardHeader className='border-bottom'>
                    <CardTitle tag='h4'>Danh sách danh mục trang thiết bị</CardTitle>
                    <div className='d-flex mt-md-0 mt-1'>
                        <Button className='ms-2' color='primary' onClick={handleAddModal}>
                            <Plus size={15} />
                            <span className='align-middle ms-50'>Thêm danh mục</span>
                        </Button>
                    </div>
                </CardHeader>
                ...
                <div className='react-dataTable whole-table__equipmentDM'>
                    <DataTable
                        noHeader
                        pagination
                        selectableRowsNoSelectAll
                        columns={ListEquipmentDMColumns}
                        className='react-dataTable table__equipmentDM'
                        paginationPerPage={10}
                        sortIcon={<ChevronDown size={10} />}
                        paginationDefaultPage={dataPage.pageNumber}
                        paginationComponent={CustomPagination}
                        // data={dataPage.querySearch.length ? filteredData.list : listEquipmentType.list}
                        data={listEquipmentType.list}
                    />
                </div>
            </Card>
            <AddEquipmentDM open={addModal} handleAddModal={handleAddModal} getData={getData} />
            {editEquipmentType && <EditEquipmentDM open={editModal} handleCloseModal={handleCloseModal} handleEditModal={handleEditModal} editEquipmentType={editEquipmentType} getData={getData} />}
            {/* <DeleteEquipmentDM open={deleteModal} handleDeleteModal={handleDeleteModal} /> */}
        </>
    )
}

export default ListEquipmentDM
